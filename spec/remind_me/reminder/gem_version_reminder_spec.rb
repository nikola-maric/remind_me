...
RSpec.describe RemindMe::Reminder::GemVersionReminder do
  ...
  let!(:all_installed_gems) { Gem::Specification.map { |a| [a.name, a.version] }.to_h }
  let!(:installed_gem_name) { 'bundler' }
  let!(:relevant_gem_version) do
    all_installed_gems[installed_gem_name].dup
  end
  let!(:relevant_gem_version_string) do
    relevant_gem_version.canonical_segments.join('.')
  end
  let!(:relevant_gem_greater_version_string) do
    relevant_gem_version.bump.canonical_segments.join('.')
  end
  let!(:relevant_gem_lower_version_string) do
    "0.1"
  end

  describe '#applicable_to_ast?' do
    it 'returns true because `gem` is present, as string' do
      comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '2.4.2', 'condition' => 'gt', message: 'Message 8'}")
    ...
    it 'returns false because `gem` is not present' do
      comment_ast = parse_string("{ '_gem' => :#{installed_gem_name},  version: '2.4.2', 'condition' => 'gt', message: 'Message 8'}")
      expect(described_class.applicable_to_ast?(comment_ast)).to eq(false)
    end
  end
  ...
  describe 'ast validations' do
    context 'version validations' do
      ...
      it 'returns validation error because version is given, but key is not string or symbol' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version => 2, 'condition' => 'gt', message: 'Message 8'}")
        expect(described_class.validate_hash_ast_version(comment_ast, dummy_location))
          .to eq(
      ...
      it 'does not return validation error because proper values have been set' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '#{relevant_gem_lower_version_string}', 'condition' => 'gt', message: 'Message 8'}")
    ...
    context 'message validations' do
      ...
      it 'returns validation error because message is given, but value is not string' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '#{relevant_gem_lower_version_string}', 'condition' => 'gt', message: 2}")
        expect(described_class.validate_hash_ast_message(comment_ast, dummy_location))
          .to eq(
    ...
    context 'condition validations' do
      ...
      it 'returns validation error because condition is given as string, but value is not string or symbol' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '#{relevant_gem_lower_version_string}', 'condition' => 2, message: '2'}")
        expect(described_class.validate_hash_ast_condition(comment_ast, dummy_location))
          .to eq(
      ...
      it 'does not return validation error because proper values have been set (from AST perspective) - string' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '#{relevant_gem_lower_version_string}', 'condition' => 'bla', message: 'Message 8'}")
        expect(described_class.validate_hash_ast_condition(comment_ast, dummy_location)).to eq(nil)
      end
  ...
  describe 'getting values from AST (reminder created must be valid)' do
    context 'getting gem values' do
      it 'gets value because key is string' do
        comment_ast = parse_string("{ 'gem' => :#{installed_gem_name},  version: '#{relevant_gem_lower_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_gem).to eq(installed_gem_name.to_sym)
      end
    ...
    context 'getting version values' do
      it 'gets value because key is string' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  'version' => '2', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
      ...
      it 'gets value because key is symbol' do
        comment_ast = parse_string("{ gem: '#{installed_gem_name}',  version: '#{relevant_gem_lower_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_version).to eq(relevant_gem_lower_version_string)
      end
      it 'gets empty string because value of hash is empty string and there are no default values' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: '', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
    ...
    context 'getting condition values' do
      it 'gets default value because value of hash is empty string' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: '#{relevant_gem_lower_version_string}', condition: '', message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_condition).to eq(:eq)
      end
      it 'gets default value because key/value pair is missing' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_condition).to eq(:eq)
      end
    ...
    context 'getting message values' do
      ...
      it 'gets default value because value of hash is empty string' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: '#{relevant_gem_lower_version_string}', 'condition' => :gt, message: ''}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_message).to eq('Condition met!')
      end
      it 'gets default value because key/value pair is missing' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', 'condition' => :gt }")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.hash_message).to eq('Condition met!')
      end
  ...
  describe '#build_from' do
    context 'values are not valid so invalid reminder is returned' do
      it 'gem value is nil' do
        comment_ast = parse_string("{ 'gem' => nil,  version: '#{relevant_gem_lower_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
      ...
      it 'gem value is empty string, therefore not installed' do
        comment_ast = parse_string("{ 'gem' => '',  version: '#{relevant_gem_lower_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'version is not a string' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: nil, condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'version key/value pair is missing entirely' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', condition: :lt, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'condition (string key) value is not one of predefined ones' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  'version' => '2', 'condition' => :bla, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'condition value is nil' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: '#{relevant_gem_lower_version_string}', condition: nil, message: 'Message 8'}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'message value is nil' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}',  version: '#{relevant_gem_lower_version_string}', 'condition' => :gt, message: nil}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
      ...
      it 'message value is not a string' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', 'condition' => :gt, message: 42}")
        subject = described_class.build_from(comment_ast, dummy_location)
        expect(subject.class).to eq(RemindMe::Reminder::InvalidReminder)
  ...
  describe 'reminder validations' do
    ...
    context 'returns validation error because gem is not installed' do
      ...
      it 'creates validation message because specified gem is not installed' do
        comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_lower_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.validation_errors).to match_array(
          [
      ...
      it 'returns empty array because gem is installed' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :lt, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
    ...
    context 'supplied condition is not valid' do
      it 'returns validation error because supplied condition is not valid' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :bla, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.validation_errors).to match_array([
    ...
    context 'supplied version is not valid' do
      it 'returns validation error because supplied version is not valid' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: 'not-a-version-string', condition: :lt, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.validation_errors).to match_array([
          "REMIND_ME comment in dummy_code_source_location mentions '#{installed_gem_name}' gem, but version specified: "\
  ...
  describe '#conditions_met?' do
    context 'version value is blank/nil, searching for any version' do
      ...
      it 'returns true because gem is not installed and version value is nil' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: nil, condition: :eq, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.conditions_met?).to eq(true)
      end
      it 'returns true because gem is not installed and version value is blank' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '', condition: :eq, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.conditions_met?).to eq(true)
      end
      it 'returns true because gem is not installed and version key/value pair is missing' do
        comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', condition: :eq, message: 'Message 8'}")
        subject = described_class.new(comment_ast, 'dummy_code_source_location')
        expect(subject.conditions_met?).to eq(true)
      end
    end
    ...
    context 'version is specified properly' do
      context 'using :lt comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', condition: :lt, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        ...
        it 'returns true because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', condition: :lt, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
      end
      context 'using :lte comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', condition: :lte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is same as target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :lte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
        it 'returns false because installed gem version is greater than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', condition: :lte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', condition: :lte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
      end
      context 'using :eq comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', condition: :eq, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is same as target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :eq, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
        it 'returns false because installed gem version is greater than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', condition: :eq, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns false because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', condition: :eq, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
      end
      context 'using :gte comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', condition: :gte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is same as target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :gte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
        it 'returns true because installed gem version is greater than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_lower_version_string}', condition: :gte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
        it 'returns false because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', condition: :gte, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
      end
      context 'using :gt comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', condition: :gt, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns false because installed gem version is same as target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', condition: :gt, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is greater than target version' do
        ...
        it 'returns false because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', condition: :gt, message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
      end
      context 'using default :eq comparison' do
        it 'returns false because gem is not installed' do
          comment_ast = parse_string("{ 'gem' => 'non-existent-gem', version: '#{relevant_gem_greater_version_string}', message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
        it 'returns true because installed gem version is same as target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_version_string}', message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(true)
        end
        it 'returns false because installed gem version is greater than target version' do
        ...
        it 'returns true because installed gem version is less than target version' do
          comment_ast = parse_string("{ 'gem' => '#{installed_gem_name}', version: '#{relevant_gem_greater_version_string}', message: 'Message 8'}")
          subject = described_class.new(comment_ast, 'dummy_code_source_location')
          expect(subject.conditions_met?).to eq(false)
        end
      end
